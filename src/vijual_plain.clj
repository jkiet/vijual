...
(defn wrap
  ...
  (letfn [(f [text]
            (lazy-seq (if (<= (count text) width)
                        [text]
                        (let [spc (->> (take width text)
                                       (map-indexed #(when (= \space %2) %1))
                                       (remove nil?))]
                          (if (seq spc)
...
(defn layout-tree [{:keys [row-padding height width width-fn] :as dim} tree]
  ...
  (let [rows (make-rows tree)
        ...
        left 0
        divider (->> widths
                     (map-indexed #(when (= total-width %2) %1))
                     (remove nil?)
                     first)
                  ...
                  (mapcat (fn [{:keys [legs dest arrow]}]
                            (let [shapes (map (fn [{x1 :xpos y1 :ypos dir1 :dir} {x2 :xpos y2 :ypos dir2 :dir}]
                                                (if (or (horizontal dir1) (vertical dir2))
                                                  {:type :rect :x (min x1 x2) :y (min y1 y2) :width (+ (Math/abs ^double (- x2 x1)) line-wid) :height line-wid}
                                                  {:type :rect :x (min x1 x2) :y (min y1 y2) :width line-wid :height (+ (Math/abs ^double (- y2 y1)) line-wid)}))
                                              legs
                                              (next legs))
...
(defn tension [edges pos]
  ...
  (apply +
         (map (fn [[f t]]
                (let [{fx :x fy :y} (pos f)
                      {tx :x ty :y} (pos t)]
                  (+ (Math/abs ^double (- fx tx)) (Math/abs ^long (- fy ty)))))
              edges)))
...
(defn tension-directed
  ...
  (apply +
         (map (fn [[f t]]
                (let [{fx :x fy :y} (pos f)
                      {tx :x ty :y} (pos t)]
                  (* (+ (Math/abs ^double (- fx tx)) (Math/abs ^double (- fy ty)))
                     (cond (> fy ty) 2
                           (= fy ty) 1
                           true 1))))
              edges)))
                     ...
                     (if-let [[[id link-index leg-index] & more] (seq lin)]
                       (recur more
                              (let [path [id :links link-index :legs]
                                    ...
                                    width (+ (Math/abs ^double (- xpos2 xpos1)) line-wid)
                                    nuy (scan-lowest-y scan xpos width)
                                    f (fn [nodes i]
